...
from generate.briefs import generate_brief_matrix


def test_generate_brief_matrix_count():
    briefs = generate_brief_matrix()
    assert len(briefs) == 54  # 3 segments x 2 goals x 3 offers x 3 tones


def test_brief_matrix_coverage():
    briefs = generate_brief_matrix()
    segments = {b.audience_segment for b in briefs}
    assert segments == {"anxious_parents", "stressed_students", "comparison_shoppers"}
